...
- Use literal arguments in direct Docker commands; shell-variable expansion and command substitution in a command containing \`docker\` are blocked. Linux Docker services run outside the visual sandbox's network namespace, so a Docker-published host or bridge address may remain unreachable even in the same bash call. If that happens, preserve network isolation: run \`SOCKET=$(pi-worker-docker-bridge start <compose-network> <service-name> <container-port>)\`, then start \`socat TCP-LISTEN:<selected-port>,bind=127.0.0.1,fork UNIX-CONNECT:$SOCKET\` inside the sandbox. The controller-owned bridge validates and mounts only the current private runtime directory; direct Docker host mounts, host networking, privileged containers, and socket forwarding remain forbidden. Use a cleanup trap that closes the browser and local socat process, runs \`pi-worker-docker-bridge stop\`, and stops the application.
