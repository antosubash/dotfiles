...
function resolveAgentModel(value: unknown): string | undefined {
	...
	for (const [tier, model] of Object.entries(CLAUDE_MODEL_ALIASES)) {
		if (normalized === tier || normalized.includes(`claude-${tier}`)) return model;
	}
	return value;
}
...
function loadAgentsFromDir(dir: string, source: "user" | "project"): AgentConfig[] {
	...
	for (const entry of entries) {
		...
		agents.push({
			...
			model: resolveAgentModel(frontmatter.model),
			systemPrompt: body,
			source,
			filePath,
		});
