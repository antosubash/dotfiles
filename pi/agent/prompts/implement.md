...
After the chain finishes, inspect the resulting diff and run missing targeted checks from `WORK_CWD`. Read and execute `issue-qa` for every issue/task: dispatch `issue-verifier` independently of `worker`, following the exact saved `/pi-plan` checklist when the user requested one, otherwise the ordinary acceptance/regression QA flow. Planning is not a prerequisite. For Figma links additionally dispatch the separate `design-verifier` through `figma-verify`. Inspect structured reports, command logs, and required screenshots; both applicable gates must PASS the current source before completion. Return mismatches to `worker` and reverify within a maximum of 3 verification passes; stop on blockers or at the bound. Do not commit or push. Preserve the worktree for resume/audit and summarize its absolute path, branch, changed files, validation, and remaining risks.
