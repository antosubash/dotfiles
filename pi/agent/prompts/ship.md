...
Parse feature text and flags. Defaults: base from origin HEAD then main/master; depth normal; review effort high; max outer rounds 3; max review passes 3.
...
**Initial baseline review (only when no prior clean review exists):** dispatch `reviewer` once against `<stage-target>...HEAD` plus uncommitted changes, project instructions, surrounding code, and tests. Ask for concrete findings only, each with severity and evidence. Save the complete output. Add P0/P1 findings to the ledger as `open`; add P2/P3 as `advisory`. If there are no open blockers, Stage A is clean even when advisories exist.

**Later outer rounds:** code before `last_clean_review_sha` is already reviewed. Dispatch `reviewer-fast` against only `last_clean_review_sha...HEAD` and the ledger entries affected by that delta. This is a comprehensive review of the new QA/fix delta, not a new audit of unchanged branch history.

For each open-blocker fix pass, up to the default maximum of 3 total review passes:

1. Record `FIX_BASE=$(git rev-parse HEAD)` and dispatch `worker` with only the exact open P0/P1 ledger entries. Run targeted checks, guard secrets, and commit the fixes.
2. Dispatch `reviewer-fast` against `FIX_BASE...HEAD` plus the open ledger entries. Its mandate is to verify each blocker and inspect the fix delta for concrete P0/P1 regressions. It must not broaden into unchanged code or promote new P2 hardening ideas into blockers.
...
4. When no blockers remain, record current HEAD as `last_clean_review_sha`, mark Stage A clean, and stop reviewing.
